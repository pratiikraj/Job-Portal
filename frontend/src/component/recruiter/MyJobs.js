      ...
      <Grid container>
        <Grid container item xs={9} spacing={1} direction="column">
          ...
          {job.website && (
            <Grid item>
              Company Website:{" "}
              <a href={job.website.startsWith("http") ? job.website : `https://${job.website}`} target="_blank" rel="noopener noreferrer" style={{color: "#3f51b5"}}>
                {job.website}
              </a>
            </Grid>
          )}
          ...
          <Grid item>
            {isNaN(parseInt(job.maxPositions))
              ? `Positions Available: ${job.maxPositions}`
              : `Remaining Number of Positions: ${parseInt(job.maxPositions) - job.acceptedCandidates}`}
          </Grid>
          <Grid item>
          ...
          >
            ...
            <Grid item>
              <TextField
                label="Any Bond"
                variant="outlined"
                value={jobDetails.bond}
                onChange={(event) => {
                  handleInput("bond", event.target.value);
                }}
                fullWidth
              />
            </Grid>
