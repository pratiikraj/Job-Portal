...
router.get("/jobs", jwtAuth, (req, res) => {
  ...
  if (req.query.asc) {
    if (Array.isArray(req.query.asc)) {
      req.query.asc.map((key) => {
        sortParams = {
          ...sortParams,
          [key === "salary" ? "salaryNumeric" : key]: 1,
        };
      });
    } else {
      sortParams = {
        ...sortParams,
        [req.query.asc === "salary" ? "salaryNumeric" : req.query.asc]: 1,
      };
    }
  }
  ...
  if (req.query.desc) {
    if (Array.isArray(req.query.desc)) {
      ...
      sortParams = {
        ...sortParams,
        [req.query.desc === "salary" ? "salaryNumeric" : req.query.desc]: -1,
      };
    }
  }
  ...
  if (user.type == "recruiter") {
    Recruiter.findOne({ userId: user._id })
      .then((recruiter) => {
        ...
        if (data.jobDescription) {
          recruiter.jobDescription = data.jobDescription;
        }
        if (data.department) {
...
// delete user profile
router.delete("/user", jwtAuth, (req, res) => {
  const user = req.user;
  if (user.type === "recruiter") {
    Recruiter.findOneAndDelete({ userId: user._id })
      .then((recruiter) => {
        if (recruiter == null) {
          res.status(404).json({
            message: "User does not exist",
    ...
    JobApplicant.findOneAndDelete({ userId: user._id })
      .then((jobApplicant) => {
        if (jobApplicant == null) {
          res.status(404).json({
            message: "User does not exist",
        ...
        .then((application) => {
          ...
          Job.findOne({
            ...
            Application.countDocuments({
              ...
              const maxPos = parseInt(job.maxPositions);
              if (isNaN(maxPos) || activeApplicationCount < maxPos) {
                // accepted
